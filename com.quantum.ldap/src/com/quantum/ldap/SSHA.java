...
/**
 * The SSHA encoding class
 */
public class SSHA {
	
	static Base64 decoder = new Base64();
	...
	public static SSHA SHA3 = new SSHA("SHA-512");
...
//		return label + new String(enc.encode(concatenate(pwhash, salt)));
		return label + new String(Base64.encodeBase64(concatenate(pwhash, salt)));
	}
...
//			hs = split(dec.decodeBuffer(digest), size);
			hs = split(Base64.decodeBase64(digest), size);
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
		byte[] hash = hs[0];
